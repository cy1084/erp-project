...
public class OffUseInfoController {
		...
		public String offInfo(Model model,@ModelAttribute OffUseInfoVO offInfoVO) {
			OffUseInfoVO off = offUseInfoService.getOffUseInfo(offInfoVO);
		...
		public String addOffUseInfo(Model model,@ModelAttribute OffUseInfoVO offInfoVO, HttpSession session) {
			...
			model.addAttribute("url","/off-use/view");
			UserInfoVO user = (UserInfoVO) session.getAttribute("user");
			offInfoVO.setOiNum(user.getOiNum());
			if(offUseInfoService.insertOffUseInfo(offInfoVO)) {
